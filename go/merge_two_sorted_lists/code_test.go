...
func TestMergeTwoLists(t *testing.T) {
	tests := []struct {
		...
		{&ListNode{Val: 1, Next: &ListNode{Val: 2, Next: &ListNode{Val: 3}}}, &ListNode{Val: 2, Next: &ListNode{Val: 3}}, &ListNode{Val: 1, Next: &ListNode{Val: 2, Next: &ListNode{Val: 2, Next: &ListNode{Val: 3, Next: &ListNode{Val: 3}}}}}},
	}
	...
	for _, test := range tests {
		...
		if !result.Equal(test.expected) {
			...
			t.Errorf("mergeTwoLists(%s, %s) = %s; expected %s", test.list1, test.list2, result, test.expected)
		}
	}
}
...
func getLists() (ListNode, ListNode) {
